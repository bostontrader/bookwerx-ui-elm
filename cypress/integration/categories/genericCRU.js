...
module.exports = ({bwURL, collName, cy}) => {
  ...
  addNew({cy, collName, newDoc: testData.categoryExpense})

  // Starting from the index, do we have exactly one document now? Are the fields correct?
  verifyIndexPage({bwURL, collName, cy, expectedCnt: 1, verifyDoc: testData.categoryExpense})

  // Navigate to the add form, add a new document, and navigate back to the index.
  addNew({cy, collName, newDoc: testData.categoryAsset})
  ...
  // 2.1 We should presently be on the /categories index page. Follow the UI, pick the first category in the list, and attempt to edit it.
  cy.get('tbody').find('tr').first().find('a').click()
  cy.get('div#categories-edit')
  cy.get('input').should('have.value', testData.categoryExpense.title)

  // 2.2 Now try to retrieve a well-formed, but non-existent id
  cy.visit(bwURL + '/#ui/' + collName + '/666666666666666666666666')
  cy.get('.loader')
  cy.get('div#errors')
  cy.get('tbody > tr').first().contains('category 666666666666666666666666 does not exist')
  ...
  cy.get('a#categories-index').click()

  // 3.2 Navigate to the edit page
  cy.get('tbody').find('tr').first().find('a').click()

  // 3.3 Update a field
  cy.get('input#title').clear().type(testData.categoryAsset.title, {delay: 25})
  ...
  cy.get('a#' + collName + '-index').click()
  cy.get('tbody').first().contains(testData.categoryAsset.title)
}
