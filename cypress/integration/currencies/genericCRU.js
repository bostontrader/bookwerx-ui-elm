...
const verifyIndexPage = ({collName, cy, expectedCnt, verifyDoc}) => {
  ...
  if (expectedCnt === 0) {
    ...
    if (verifyDoc) {
      cy.get('tbody').first().contains(verifyDoc.symbol)
...
const addNew = ({cy, collName, newDoc}) => {
  ...
  cy.get('input#symbol').type(newDoc.symbol, {delay: 25})
  cy.get('input#title').type(newDoc.title, {delay: 25})
...
module.exports = ({bwURL, collName, cy}) => {
  ...
  addNew({cy, collName, newDoc: testData.currencyCNY})

  // Starting from the index, do we have exactly one document now? Are the fields correct?
  verifyIndexPage({bwURL, collName, cy, expectedCnt: 1, verifyDoc: testData.currencyCNY})

  // Navigate to the add form, add a new document, and navigate back to the index.
  addNew({cy, collName, newDoc: testData.currencyRUB})
  ...
  cy.get('div#currencies-edit')
  cy.get('input#symbol').should('have.value', testData.currencyCNY.symbol)
  cy.get('input#title').should('have.value', testData.currencyCNY.title)

  // 2.2 Now try to retrieve a well-formed, but non-existent id
  cy.visit(bwURL + '/#ui/' + collName + '/666666666666666666666666')
  cy.get('.loader')
  cy.get('div#errors')
  cy.get('tbody > tr').first().contains('currency 666666666666666666666666 does not exist')
  ...
  cy.get('a#currencies-index').click()

  // 3.2 Navigate to the edit page
  cy.get('tbody').find('tr').first().find('a').click()

  // 3.3 Update the fields
  cy.get('input#symbol').clear().type(testData.currencyRUB.symbol, {delay: 25})
  cy.get('input#title').clear().type(testData.currencyRUB.title, {delay: 25})
  ...
  // 3.6. Now return to collection-index and verify that the field has been changed.
  cy.get('a#' + collName + '-index').click()
  cy.get('tbody').first().contains(testData.currencyRUB.symbol)
  cy.get('tbody').first().contains(testData.currencyRUB.title)
}
