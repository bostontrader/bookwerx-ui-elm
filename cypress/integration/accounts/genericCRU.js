...
module.exports = ({bwURL, collName, cy}) => {
  ...
  verifyIndexPage({bwURL, collName, cy, expectedCnt: 1, verifyDoc: testData.accountBank})
  ...
  // 3.3 Update a field
  cy.get('input#title').clear().type(testData.accountCash.title, {delay: 25})
  ...
  cy.get('a#' + collName + '-index').click()
  cy.get('tbody').first().contains(testData.accountCash.title)
}
