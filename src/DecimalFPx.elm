...
dfp_addx n1 n2 =
    ...
    in
    if d >= 1 then
        dfp_addx { amount = n1.amount * 10, exp = n1.exp - 1 } n2
...
dfp_fmtx p dfp =
    ...
    in
    -- If dfp_norm != norm(dfp) then there are digits obscured by rounding.
    DFPFmtx s4 (not (dfp_equalx dfp_norm (norm dfp)))
...
dfp_roundx p d =
    if p <= d.exp then
        ...
        let
            ...
            new_dfp =
                DFPx new_amount (d.exp + 1)
        in
        dfp_roundx p new_dfp
