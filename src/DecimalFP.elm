...
dfp_equal d1 d2 = d1.sig == d2.sig && d1.exp == d2.exp
...
dfp_fmt p dfp =
    ...
    let
        ...
        slen = String.length samt


        s2 = if dfp_norm.exp < 0 then -- move dp to the left
...
round p d =
    if p <= d.exp then
        ...
        let
            last_digit = modBy 10 d.sig
            new_sig = if last_digit >= 5 then
                d.sig // 10 + 1
                else
                    d.sig // 10

            new_dfp = DFP new_sig (d.exp + 1)
        in
            round p new_dfp
